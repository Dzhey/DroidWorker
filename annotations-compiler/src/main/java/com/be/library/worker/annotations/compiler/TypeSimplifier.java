...
final class TypeSimplifier {
     ...
     * the package containing the AutoParcel class, provided that the result refers to the type
    ...
    // If we have @AutoParcel abstract class Foo<T extends SomeClass> then this method will
    ...
    static TypeElement enclosingClass(Element elem) {
        Element result;
        Element encl;
        for(result = elem; result != null && !result.getKind().isClass() && !result.getKind().isInterface(); result = encl) {
            encl = result.getEnclosingElement();
        }
    ...
    static PackageElement enclosingPackage(Element elem) {
        Element result;
        Element encl;
        for(result = elem; result != null && result.getKind() != ElementKind.PACKAGE; result = encl) {
            encl = result.getEnclosingElement();
        }
